...
2. 0부터 행의 갯수만큼 반복하여 외각의 값인 board[i][0]과 board[i][$board[0].legnth - 1$]의 값이 'O'인지 검증하여 그 옆의 값 위주로 검증하여 주변이 'X'인 'O'만 'o'로 변경하여 대상을 표시한다.

3. 0부터 열의 갯수만큼 반복하여 외각의 값인 board[0][i]]과 board[$board.legnth - 1$][i]의 값이 'O'인지 검증하여 그 옆의 값 위주로 검증하여 주변이 'X'인 'O'만 'o'로 변경하여 대상을 표시한다.
